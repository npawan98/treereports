...
function Park_2Table() {
    ...
    useEffect(()=>{
        axios.get("https://afternoon-mountain-93762.herokuapp.com/Park_2Evergreen").then(res=> {
    ...
    axios.get("https://afternoon-mountain-93762.herokuapp.com/Park_2Endemic").then(res=> {
        console.log(res.data)
        setPark_2Endemic(res.data)
    })
    axios.get("https://afternoon-mountain-93762.herokuapp.com/Park_2Indigenous").then(res=> {
        console.log(res.data)
        setPark_2Indigenous(res.data)
    })
    axios.get("https://afternoon-mountain-93762.herokuapp.com/Park_2Exotic").then(res=> {
        console.log(res.data)
        setPark_2Exotic(res.data)
    })
    axios.get("https://afternoon-mountain-93762.herokuapp.com/Park_2Endangered").then(res=> {
        console.log(res.data)
    ...
    <TableContainer component={Paper} style={{maxWidth:'650px'}}>
      <Table sx={{ maxWidth: 650 }} aria-label="simple table">
        ...
        <TableRow onClick={()=>{window.location.assign("/Park_2_Park/Park_2_Park-evergreen")}} style={{cursor:"pointer"}}>
          ...
          <TableRow onClick={()=>{window.location.assign("/Park_2_Park/Park_2_Park-endemic")}}>
            <TableCell>Number of endemic </TableCell>
            <TableCell align="right">{Park_2Endemic.length}</TableCell>
          </TableRow>
          <TableRow onClick={()=>{window.location.assign("/Park_2_Park/Park_2_Park-indigenous")}}>
            <TableCell>Number of indigenous</TableCell>
            <TableCell align="right">{Park_2Indigenous.length}</TableCell>
          </TableRow>
          <TableRow onClick={()=>{window.location.assign("/Park_2_Park/Park_2_Park-exotic")}}>
            <TableCell>Number of exotic</TableCell>
            <TableCell align="right">{Park_2Exotic.length}</TableCell>
          </TableRow>
          <TableRow onClick={()=>{window.location.assign("/Park_2_Park/Park_2_Park-endangered")}}>
